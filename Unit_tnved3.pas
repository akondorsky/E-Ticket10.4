...
function TFormTnved3.FindTovname(Fstr:String):Boolean;
begin
 Result:=false;
...
procedure TFormTnved3.DBGridEh1DblClick(Sender: TObject);
begin
ModalResult:=mrOk;
end;

procedure TFormTnved3.E_FindButtonClick(Sender: TObject);
...
procedure TFormTnved3.ToolButton5Click(Sender: TObject);
 var
  s:String;
begin
  if Length(Trim(E_Find.Text)) =0 then Exit;
...
procedure TFormTnved3.ToolButton4Click(Sender: TObject);
 var
  R:Integer;
begin
...
procedure TFormTnved3.E_FindKeyPress(Sender: TObject; var Key: Char);
begin
  if Key=#13 then E_FindButtonClick(E_Find as TComboEdit);
end;

procedure TFormTnved3.E_FindChange(Sender: TObject);
begin
